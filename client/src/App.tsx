...
import { Container, Typography, Box, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, TextField, MenuItem, IconButton, Tooltip } from '@mui/material';
...
function App() {
  ...
  const handleOpen = (training?: Training) => {
    if (training) {
      setForm({ ...training, startDate: training.startDate.slice(0, 10), endDate: training.endDate.slice(0, 10) });
  ...
  const filteredTrainings = trainings.filter((t) => {
    return (
      ...
      (!filters.percentCompleted || String(t.percentCompleted ?? '').includes(filters.percentCompleted)) &&
        ...
        <Container maxWidth="md" sx={{ mt: 6 }}>
                ...
                <Table sx={{ minWidth: 1170 }}>
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell sx={{ width: '18.97%', backgroundColor: '#006A71', color: '#fff', fontWeight: 'bold', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Training Type</TableCell>
                      <TableCell sx={{ width: '12.52%', backgroundColor: '#006A71', color: '#fff', fontWeight: 'bold', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Start Date</TableCell>
                      <TableCell sx={{ width: '12.52%', backgroundColor: '#006A71', color: '#fff', fontWeight: 'bold', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>End Date</TableCell>
                      <TableCell sx={{ width: '10%', backgroundColor: '#006A71', color: '#fff', fontWeight: 'bold', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Status</TableCell>
                      <TableCell sx={{ width: '10%', backgroundColor: '#006A71', color: '#fff', fontWeight: 'bold', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>% Completed</TableCell>
                      <TableCell align="right" sx={{ width: '5.46%', backgroundColor: '#006A71', color: '#fff', fontWeight: 'bold', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Actions</TableCell>
                    </TableRow>
                    <TableRow>
                      ...
                      <TableCell><TextField size="small" select variant="outlined" placeholder="Filter" value={filters.trainingType} onChange={e => setFilters(f => ({ ...f, trainingType: e.target.value }))} fullWidth><MenuItem value="">All</MenuItem>{trainingTypeOptions.map(option => (<MenuItem key={option} value={option}>{option}</MenuItem>))}</TextField></TableCell>
                      <TableCell><TextField size="small" type="date" variant="outlined" value={filters.startDate} onChange={e => setFilters(f => ({ ...f, startDate: e.target.value }))} fullWidth InputLabelProps={{ shrink: true }} /></TableCell>
                      <TableCell><TextField size="small" type="date" variant="outlined" value={filters.endDate} onChange={e => setFilters(f => ({ ...f, endDate: e.target.value }))} fullWidth InputLabelProps={{ shrink: true }} /></TableCell>
                      <TableCell><TextField size="small" select variant="outlined" value={filters.status} onChange={e => setFilters(f => ({ ...f, status: e.target.value }))} fullWidth><MenuItem value="">All</MenuItem>{statusOptions.map(option => (<MenuItem key={option} value={option}>{option}</MenuItem>))}</TextField></TableCell>
                      <TableCell><TextField size="small" variant="outlined" placeholder="Filter" value={filters.percentCompleted} onChange={e => setFilters(f => ({ ...f, percentCompleted: e.target.value }))} fullWidth /></TableCell>
                      <TableCell />
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {filteredTrainings.map((t) => (
                      <TableRow key={t._id} sx={{ height: 24, '&:not(:last-child)': { borderBottom: '1px solid #e0e0e0' }, '& > *': { paddingTop: 0, paddingBottom: 0 }, backgroundColor: '#9ACBD0' }}>
                        ...
                        <TableCell sx={{ width: '18.97%', padding: '0 8px' }}>{t.trainingType}</TableCell>
                        <TableCell sx={{ width: '12.52%', padding: '0 8px' }}>{new Date(t.startDate).toLocaleDateString()}</TableCell>
                        <TableCell sx={{ width: '12.52%', padding: '0 8px' }}>{new Date(t.endDate).toLocaleDateString()}</TableCell>
                        <TableCell sx={{ width: '10%', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', padding: '0 8px' }}>{t.status}</TableCell>
                        <TableCell sx={{ width: '10%', padding: '0 8px' }}>{typeof t.percentCompleted === 'number' ? `${t.percentCompleted}%` : ''}</TableCell>
                        <TableCell align="right" sx={{ width: '5.46%', padding: '0 8px' }}>
                          <Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', gap: 1 }}>
                            <IconButton aria-label="edit" onClick={() => { setShowForm(true); setShowTable(false); handleOpen(t); }} size="small"><EditIcon fontSize="small" /></IconButton>
                            <IconButton aria-label="delete" onClick={() => handleDelete(t._id!)} size="small"><DeleteIcon fontSize="small" /></IconButton>
                          </Box>
                        </TableCell>
                      </TableRow>
                    ))}
          ...
          {showForm && (
            <Box component="form" sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 4, background: 'linear-gradient(to right,rgb(221, 227, 236),rgb(190, 168, 240),rgb(241, 177, 209) 100%)', p: 3, borderRadius: 2, boxShadow: 2, width: '200%', minWidth: 350, alignSelf: 'center' }}>
              ...
              <TextField select label="Training Type" name="trainingType" value={form.trainingType} onChange={handleChange} required sx={{ width: '100%' }}>
                {trainingTypeOptions.map((option) => (
                  <MenuItem key={option} value={option}>{option}</MenuItem>
                ))}
              </TextField>
              <TextField label="Start Date" name="startDate" type="date" value={form.startDate} onChange={handleChange} InputLabelProps={{ shrink: true }} required sx={{ width: '100%' }} />
              <TextField label="End Date" name="endDate" type="date" value={form.endDate} onChange={handleChange} InputLabelProps={{ shrink: true }} required sx={{ width: '100%' }} />
              <TextField select label="Status" name="status" value={form.status} onChange={handleChange} required sx={{ width: '100%' }}>
                {statusOptions.map((option) => (
                  <MenuItem key={option} value={option}>{option}</MenuItem>
                ))}
              </TextField>
              <TextField label="% Completed" name="percentCompleted" type="number" value={form.percentCompleted ?? ''} onChange={handleChange} inputProps={{ min: 0, max: 100 }} required sx={{ width: '100%' }} />
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, mt: 2 }}>
                <Button onClick={() => { setShowForm(false); setShowTable(true); handleClose(); }}>Cancel</Button>
